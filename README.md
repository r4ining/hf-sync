...
Hugging Face 和 ModelScope 各自提供了命令行工具（`hf`、`modelscope`），
...
- **本地目录支持**：任一端可替换为本地目录，作为纯下载 / 上传工具使用（使用体验可能不如直接使用 `hf` 或 `modelscope`）
...
`hf-sync` 的设计目标是**不需要在本地保留整个仓库的完整副本**，逐文件传输，同一时刻仅在本地暂存当前正在传输的文件。每个文件都会先写入本地临时文件（`.part`），完成后再交给目标端。
...
- **会占用本地网络流量**，且流量规模跟仓库总大小同一个量级。每个文件从源端下载一次到本地临时文件，
  再从本地临时文件上传到目标端，本地总流量约为待同步文件总大小的 **~2 倍**（源端下行 ~1x + 目标端
  上行 ~1x）。
- **会占用本地磁盘空间**，峰值约为「当前并发传输的各文件大小之和」。每个文件在传输期间会在系统临时
  目录下生成 `.part` 文件，上传成功后自动删除。如果临时目录所在磁盘空间不足，可通过环境变量 `TMPDIR`
  指向更充足的目录。
- **同步速度受限于本机的上下行带宽**。上传带宽 20Mbps（~2.5MB/s）时，
  光上传 100GB 就需要约 11 小时。`hf-sync` 默认并发 5 个文件以更好地利用带宽，但单文件较大时
  瓶颈仍在单个传输流水线，并发主要在多文件场景下生效。
- 传输中途 `Ctrl-C` 或断网时，已下载的部分会保留在本地 `.part` 文件中，下次运行相同文件时会通过
  HTTP Range 请求从断点处续传，无需重新下载。已成功上传的文件会自动跳过。
